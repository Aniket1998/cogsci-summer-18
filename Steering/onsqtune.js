...
function Locomotion(params) {
	...
	this.steer = function(force,dt, parray,target) {
		...
		if (dt > 0) {
			var smoothRate = clip_value(9 * dt,0.15,0.7); // TUNE : Figure out a good formula, using default as of now
	...
	this.steeringSeek = function(target) {
		var desire = ((target.subtract(this.position)).normalize()).multiply(this.maxSpeed);
		return (desire.subtract(this.velocity)).normalize().multiply(2*this.maxForce);
	}
	...
	this.steerToAvoidCollisions = function(parray) {
		...
		if(parray.length) {
			...
			if(threat) {
				...
				var dist =(obstacle.shape.position.subtract(this.shape.position)).length
				// Del pos = v * t for now
