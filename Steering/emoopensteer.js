...
function Locomotion(params) {
	...
	if(parray.length) {
			...
			//there exist obstacles within danger limits
			var threat = 0;
			var pspace = 6 * 15;
			...
			var dist = parray[0].position.subtract(this.position);
			var projection = dist.dot(this.vec);
			var my_future = this.position.add(this.velocity.multiply(mintime));
			var obs_future = parray[0].position.add(parray[0].velocity.multiply(mintime));
			var mindist = my_future.subtract(obs_future).length;
					...
					for(var i=1; i<parray.length; i++) {
						...
						if(time >= 0 && time < mintime) {
							my_future = this.position.add(this.velocity.multiply(mintime));
							obs_future = parray[i].position.add(parray[i].velocity.multiply(mintime));
			...
			if(threat != 0) {
					...
					my_future = this.position.add(this.velocity.multiply(mintime).add(this._smoothAcc.multiply(mintime*mintime)));
					obs_future = obstacle.position.add(obstacle.velocity.multiply(mintime).add(obstacle._smoothAcc.multiply(mintime*mintime)));
					mindist = my_future.subtract(obs_future).length;
        		...
        		if (parallelness < -angle) {
            		...
            		//steer is direction of swaying
            		
		}
		if(sep>1)
			force = force.add(this.basisPerpendicular.multiply((1000 * steer)/(sep)));
