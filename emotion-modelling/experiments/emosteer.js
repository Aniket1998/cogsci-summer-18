		...
		if(person == false) {
			...
			if (parray.length){
				...
				//check for most threatening obstacle 
					...
					console.log(obs_future + " myf");					
					mindist = my_future.subtract(obs_future).length;
					
					var parallelness = this.vec.dot(obstacle.vec);
    		    	var angle = 0.707;
    		    	var offset, sideDot, steer;
    		    	var vy = this.vec.rotate(90);
        		...
        		if (parallelness < -angle) {
            		...
            		this.finvelocity = this.finvelocity.add(vy.multiply(steer));
            	}
			} else {
				//moving target
			}	
